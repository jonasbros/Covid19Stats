<template>
    <div class="search__container">
      <form action="#">
        <select class="countries-select" autocomplete="false">
          <option value="">Search country</option>
          <option :value="c.Slug" v-for="(c, k) in countries" :key="k">{{c.Country}}</option>
        </select>
      </form>
    </div>
</template>
...
export default {
    ...
    mounted() {
        window.$('.countries-select').selectize().on('change', (e) => {
            this.$router.push({ name: 'Country', params: { slug: e.target.value, countryName: e.target.firstChild.innerHTML } });
        });
    },
}
</script>
